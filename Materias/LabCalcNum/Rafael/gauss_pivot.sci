...
for k = 1:n-1
    for i = k+1:n
        ...
        //pivoteamento
           patual=abs(Aug(k,k)) //grava o pivo atual
           ip=k //indice do pivo
           for count=k+1:n
               if  abs(Aug(count,k)) > patual then
                   patual=abs(Aug(count,k))
                   ip=count
               end
           end
           //verificar se encontrou um valor maior
           if ip ~= k then
               aux=Aug(ip,:)    //troca a linha do pivo atual pelo
               Aug(ip,:)=Aug(k,:)   //de maior valor
               Aug(k,:)=aux
           end
